...
<template>
  <div v-if="valmaTelmaSisalto">
    <template v-if="valmaTelmaSisalto.osaamistavoite && valmaTelmaSisalto.osaamistavoite.length > 0">
      <div v-for="(osaamistavoite, index) in valmaTelmaSisalto.osaamistavoite" :key="'osaamistavoite'+index" class="mb-5">
    ...
    <template v-if="valmaTelmaSisalto.osaamisenarviointiTekstina || valmaTelmaSisalto.osaamisenarviointi">
      ...
      <template v-if="valmaTelmaSisalto.osaamisenarviointi">
        <h4 class="mt-3">{{$kaanna(valmaTelmaSisalto.osaamisenarviointi.kohde)}}</h4>

        <ul>
          <li v-for="(tavoite, tindex) in valmaTelmaSisalto.osaamisenarviointi.tavoitteet" :key="'osaamisenarviointitavoite'+tindex">
            {{$kaanna(tavoite)}}
          </li>
        </ul>
      </template>
